...
class ReverseSundayPatternMatchingAlgorithm : public PatternMatchingAlgorithm {
public:
    int find_matches(const std::string &t, const std::string &p) override {
        size_t t_size = t.size();
        size_t p_size = p.size();
        ...
        while (t_index <= t_size - p_size) {
            ...
            while (p_index < p_size && p[p_index] == t[t_index + p_index]) {
                p_index++;
            }
    ...
    std::string get_name() override { return "Reverse Sunday"; }
};
...
public:
    ...
    static std::string get_name() { return "2D Rabin-Karp"; }
};
